...
class ChatService:
    ...
    def history(self, limit: int = 40) -> list[ChatMessage]:
    ...
    def turn(self, req: ChatTurnRequest) -> ChatTurnResponse:
        ...
        for hint, name in _METRIC_HINTS.items():
            if hint in lower:
                metric = name
                break
        m = re.search(r"\b([a-z_]+)\b", lower)
        if metric is None and "metric" in lower and m:
            pass
        ...
        # Always attach light evidence search for grounding
                ...
                tool_results.append(
                    {
                        ...
                        "result": {
                            "processed": False,
                            "detail": "llava available but image OCR path is opt-in; metadata only this turn.",
                            "attachment_count": len(req.attachments),
                        },
                    }
                )
